...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
    ...
    fprintf(stderr, "Learning Rate: %g, Momentum: %g, Decay: %g\n", net->learning_rate, net->momentum, net->decay);
    srand(time(0));
    //printf("inicia carga de archivo de validacion\n");
    list *plist = get_paths("/data/faculty/calderonf/Conversion_de_anotaciones_SDM_por_publicar/valid.txt");
    char **paths = (char **)list_to_array(plist);
    ...
    //printf("-%s-",paths[0]);
    //printf("<-\n");
    printf("i;correct;totalRPs/Img;;proposals/(i+1);IOU:;avg_iou*100/total;Recall:;100.*correct/total\n");
    for(i = 0; i < m; ++i){
        char *path = paths[i];
        //printf("-->>imagen:%s:",path);
