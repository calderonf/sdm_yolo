                ...
                if len(track.p.p[idx].path)>2: # si la longitud del path es mayor a dos
                    ...
                    for contar in contadores:
                        if (contar.testLine(p2,p1) and not track.p.p[idx].contadores[contar.linecount]):
                            ...
                            if (str(track.p.p[idx].str) == 'ciclista') or (str(track.p.p[idx].str) == 'peaton') or (str(track.p.p[idx].str) == 'motociclista'):
                                imfilesave=folder+"/"+track.p.p[idx].str+'_'+str(contimagen)+'_'+str(random.randint(10000,1000000))+'.JPG'
