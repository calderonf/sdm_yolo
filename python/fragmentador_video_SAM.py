...
def FFMPEG_Merge_Video_Files(path, sources_list):
    ...
    """
    Execute a FFMPEG in console to merge videos in only one result
        ...
        if image_file.endswith("avi"): 
            ...
            if len(partial_info) ==6:
                ...
                directory_list.append({ 'File_Name'      :image_file,                               # File Name
                                        ...
                                        'Random_Number'  :"VIDEODESAM",                             # Random number for file
                                        ...
                                        'Date'           :YYYY+MT+DD+HH+MM+SS,                      # YYYYMMDDHHMMSS
                                        'Camera'         :cameranum,                                # ID Camera 
                                        'YYYY'           :YYYY,                                     # Year
                                        'MT'             :MT,                                       # Month
                                        'DD'             :DD,                                       # Day
                                        'HH'             :HH,                                       # Hour
                ...
                i=i+1
            elif len(partial_info) ==3:#Formato antiguo para revisar salida.
                # Split video's file name with a delimiter to get partial information 
                partial_information = image_file.split('_')
