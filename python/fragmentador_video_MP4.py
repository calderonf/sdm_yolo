...
def FFMPEG_Merge_Video_Files(path, sources_list):
    ...
    result_name = "merge_video_souces.MP4"
...
def FFMPEG_Split_Video_Files_(absolute_path, video_source_name, split_duration, delete_source):
    ...
    # ffmpeg -i input.mp4 -c copy -map 0 -segment_time tt -f segment output%03d.MP4
    cmd = 'ffmpeg -i {} -c copy -map 0 -reset_timestamps 1 -segment_time {} -f segment {}%04d.MP4'.format(os.path.join(absolute_path,video_source_name), 
...
def rename_split_results(results_path, Camera_Name, video__Start_Time, YYYYMTDD):
    ...
    for image_file in dirlist:
        ...
        if image_file.endswith("MP4"): 
            ...
            new_name = 'CAM{}_{}{}_{}.MP4'.format(Camera_Name, YYYYMTDD, video__Start_Time, i)
...
def read_sources(selected_folder):
    ...
    """
    read .MP4 files at folder and return a list with each video information
...
if __name__ == '__main__':
    ...
    # Read .MP4 files at folder and return a list with each video information (Now with video Results)
